...
def _parse_and_save_with_timeout(args: tuple[str, float]) -> PageResult:
    url, timeout = args
    return parse_and_save(url, timeout)
...
def parse_and_save(url: str, timeout: float = DEFAULT_TIMEOUT) -> PageResult:
    ...
    result = PageResult("multiprocessing", url, title, status, elapsed, record_id, error)
...
def parse_many(
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        return list(
            executor.map(
                _parse_and_save_with_timeout,
                [(url, timeout) for url in selected_urls],
            )
        )


def main() -> None:
    parser = build_parser("Parse pages with multiprocessing.")
